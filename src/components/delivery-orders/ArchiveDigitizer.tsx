          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-mono opacity-75">
                {savingProgress.orderNumber}
                {" • "}
                {isId
                  ? `Total Batch: ${rows.length} Surat Jalan • ${formatIDR(totalBatchValueIDR)}`
                  : `Batch total: ${rows.length} orders • ${formatIDR(totalBatchValueIDR)}`}
              </p>
            </div>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="18000"
                      aria-invalid={isInvalid && (!row.unitPrice || row.unitPrice <= 0)}
                      aria-label={isId ? `Harga satuan baris ${idx + 1}` : `Unit price row ${idx + 1}`}
                      className={`w-full min-h-[40px] rounded-xl border px-3 py-2 text-xs font-mono font-bold focus:outline-none focus:border-brand ${
                        isInvalid && (!row.unitPrice || row.unitPrice <= 0)
                          ? "border-red-500 ring-2 ring-red-200 dark:ring-red-950 bg-red-50/50"
                  ...
                  return (
                    ...
                    <td className="p-2">
                      <input
                        ...
                        aria-invalid={isInvalid && !row.recipientName.trim()}
                    ...
                    <td className="p-2 border-l border-gray-100 dark:border-gray-800">
                      <input
                        ...
                        className={`w-full text-right rounded-lg border px-2 py-1 font-mono font-bold text-xs tabular-nums focus:outline-none focus:border-brand ${
                          isInvalid && (!row.unitPrice || row.unitPrice <= 0)
                            ? "border-red-500 ring-2 ring-red-200 dark:ring-red-950 bg-red-50/50"
                      ...
                      return (
                        <td key={size} className="p-1.5 text-center border-l border-gray-100 dark:border-gray-800">
                          <input
                            ...
                            aria-invalid={isInvalid && rowTotal === 0 && !hasQty}
                            placeholder="·"
                            className={`w-full min-w-[48px] text-center rounded-lg border px-1.5 py-1 font-mono font-extrabold text-xs transition focus:outline-none focus-visible:ring-2 focus-visible:ring-brand tabular-nums ${
                              hasQty
                                ...
                                : isInvalid && rowTotal === 0
                                ? "border-red-400 ring-1 ring-red-200 dark:ring-red-950 bg-red-50/40"
